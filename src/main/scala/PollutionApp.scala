...
object PollutionApp extends App {
  ...
  /**
   * clean_data
   * Ouvrent les csv en fonction de leur type
   * @return DataFrame
   */
  ...
  /**
   * merge_df
   * Fusionnent la colonne d'un dataframe avec un second de même nature et structure
   * @return DataFrame
   */
  ...
  //% Total par pays en zone rurale
  basic_hand
    .filter(basic_hand("Dim1") === "Rural")
    .orderBy(asc("% basic handwashing"))
    .show()

  //% Total par pays en zone urbaine
  basic_hand
    .filter(basic_hand("Dim1") === "Urban")
    .orderBy(desc("% basic handwashing"))
    .show()
  ...
  spark.sql("SELECT * " +
    "FROM air_pollution " +
    "WHERE air_pollution.Full_value > 250").show()

  // Nombre de mort maximum les deux sexes & Nombre de mort maximum Homme ou Femme
  spark.sql("SELECT Location, Dim2,MAX(Full_value) " +
    "FROM air_pollution " +
    "WHERE air_pollution.Dim1 = 'Both sexes' " +
    "GROUP BY Location,Dim2").show()

  spark.sql("SELECT Location,Dim2,MAX(Full_value) " +
    "FROM air_pollution " +
    "WHERE air_pollution.Dim1 = 'Female' " +
    "GROUP BY Location,Dim2").show()
  ...
  mortality
    ...
    .show()
  //Nombre maximum de nettoyage par système infecté par sexe
  mortality
    .groupBy("Dim1")
    .max("Mortality UnsafeW 100K")
    .show()
  // Somme totale par sexe d'empoisonnement involontaire par et de nettoyage par système infecté par sexe
  mortality
    .groupBy("Dim1")
